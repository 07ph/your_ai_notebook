...
export function Sidebar({ onCollapse }: SidebarProps) {
  ...
  const renderGroup = (label: string, items: typeof sessions) => {
    ...
    return (
      <div className="mb-3">
        <h3 className="px-3 py-1 text-xs font-medium text-slate-400 uppercase tracking-wider">
          {label}
        </h3>
          ...
          {items.map((session) => (
            <li key={session.id} className="relative">
              ...
              {isRenaming && renameId === session.id ? (
                <div className="px-2 py-1.5">
                  <input
                    ...
                    onBlur={handleRename}
                    className="w-full px-2 py-1 text-sm bg-white border border-blue-500 rounded-md outline-none focus:ring-1 focus:ring-blue-500"
                  />
                </div>
              ) : (
                <button
                ...
                >
                  <div className="flex items-center justify-between gap-1">
                    ...
                    <span
                      ...
                      className="flex h-5 w-5 shrink-0 items-center justify-center rounded text-slate-400 hover:bg-slate-200 hover:text-slate-600 transition-colors cursor-pointer"
              ...
              {contextMenuId === session.id && (
                <div
                  ref={contextMenuRef}
                  className="absolute right-2 top-8 z-20 w-32 rounded-lg border border-slate-200 bg-white py-1 shadow-lg"
                >
                  <button
                    onClick={() => {
                    ...
                    className="flex w-full items-center gap-2 px-3 py-1.5 text-sm text-slate-700 transition-colors hover:bg-slate-50"
                  >
    ...
    <aside className="w-64 h-full bg-slate-50 border-r border-slate-200 flex flex-col">
      ...
      <div className="flex items-center gap-2 p-3">
        {isCreating ? (
          <input
            ...
            placeholder="对话名称..."
            className="flex-1 px-3 py-2 text-sm bg-white border border-blue-500 rounded-lg outline-none focus:ring-1 focus:ring-blue-500"
          />
        ) : (
          <button
        ...
        {onCollapse && (
          <button
            onClick={onCollapse}
            className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md text-slate-400 transition-colors hover:bg-slate-200 hover:text-slate-600"
            title="收起对话列表"
          >
      ...
      <div className="px-3 pb-2">
        <input
          ...
          className="w-full px-3 py-1.5 text-sm bg-white border border-slate-200 rounded-lg placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
        />
      </div>
